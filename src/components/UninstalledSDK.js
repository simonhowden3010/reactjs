...
const UninstalledSDK = () => {
  ...
  useEffect(() => {      
    getData('uninstalled.json')
      .then(res => {
        // sort by date (most recent first)
        res.data.uninstalledSdks.sort(custom_sort);
        setData(res)

        var list = [];
        res.data.uninstalledSdks.forEach(function (item) {
            ...
            <CardContent>
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} md={2}>
                  {dataLoaded === true && 
                    <Typography align="right" variant="h2" color="inherit" noWrap>{data.data.uninstalledSdks.length}</Typography>
                  }
                </Grid>
                {dataLoaded === true && 
                  <Grid item xs={12} md={12}>
                    <Typography align="left" variant="body1" color="inherit" noWrap>Last update: {getReadableDate(data.data.uninstalledSdks[0].lastSeenDate)}</Typography>
                  </Grid>
                }
                <Grid item xs={12} md={12}>
                  {dataLoaded === true && 
                          ...
                          {dataCategories.map((dataCategoryItem, outerKey) => {
                            return (
                              <Grid item xs={12} md={6} key={outerKey}>
                                ...
                                {data.data.uninstalledSdks.map((sdkItem, key) => {
